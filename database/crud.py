...
def add_scraped_post(db_conn: sqlite3.Connection, post_data: Dict, group_id: int) -> Optional[tuple[int, bool]]:
    """
    ...
    Returns:
        Tuple of (internal_post_id, is_new) if the post was successfully added or already existed,
        None otherwise.
    """
    sql = '''
...
def botsettings_get(db_conn: sqlite3.Connection, key: str, default: str = "") -> str:
    try:
        cursor = db_conn.cursor()
...
def botsettings_set(db_conn: sqlite3.Connection, key: str, value: str) -> None:
    try:
        cursor = db_conn.cursor()
        cursor.execute("INSERT INTO BotSettings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, value))
        db_conn.commit()
    except sqlite3.Error:
        db_conn.rollback()
...
if __name__ == '__main__':
    ...
    if conn:
        ...
        post_id, is_new = add_scraped_post(conn, test_post, group_id)
        logging.info(f"Adding test post, returned ID: {post_id}, is_new: {is_new}")
