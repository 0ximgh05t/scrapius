...
• <b>Import Cookies</b> works everywhere and is recommended for servers"""
        ...
        keyboard = {
            "inline_keyboard": [
                ...
                [{"text": "🖥️ Manual Browser", "callback_data": "login_manual"}],
    ...
    async def _handle_login_callback(self, callback_data: str, bot_token: str, chat_id: str, conn, answer_callback) -> None:
        ...
        if callback_data == 'login_manual':
            answer_callback("🖥️ Opening Chrome browser. Login and close when done!", show_alert=True)
            await self._start_manual_login(bot_token, chat_id)
        elif callback_data == 'login_auto':
...
Try again with the correct format."""
            ...
            try:
                ...
                # Set shorter timeouts for server environments
                driver.set_page_load_timeout(30)  # 30 second timeout
                
                cookies_loaded = load_cookies(driver, cookie_path)
                if not cookies_loaded:
                    send_telegram_message(bot_token, chat_id, "❌ <b>Failed to load cookies into browser!</b>\n\nTry importing again.", parse_mode="HTML")
                    driver.quit()
                    return
                ...
                try:
                    ...
                    logging.warning(f"Session validation failed: {session_error}")
                    session_valid = False
...
You can now start scraping!"""
                    ...
                    # Still save cookies even if validation fails (might work for scraping)
                    warning_msg = f"""⚠️ <b>Cookies imported but session validation failed!</b>

🍪 <b>Imported:</b> {len(cookies)} total cookies
🎯 <b>Facebook:</b> {len(fb_cookies)} Facebook cookies
❓ <b>Status:</b> Validation timeout/failed

<b>Possible causes:</b>
• Network connectivity issues
• Facebook blocking server IP
• Cookies expired or invalid

<b>Next steps:</b>
• Cookies are saved and may still work for scraping
• Monitor bot logs for actual scraping results
• Re-import if scraping fails"""
                    
                    send_telegram_message(bot_token, chat_id, warning_msg, parse_mode="HTML")
                    
            except Exception as test_error:
                logging.error(f"Cookie testing error: {test_error}")
                send_telegram_message(bot_token, chat_id, f"❌ <b>Cookie testing failed:</b> {str(test_error)}\n\nCookies are saved but validation failed. Monitor logs for scraping results.", parse_mode="HTML")
